...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--merger-brief",
        default="",
        help="Optional plain-language instructions for the merged product outcome.",
    )
    parser.add_argument(
        "--merger-brief-file",
        default=None,
        type=Path,
        help="Optional file containing merger instructions and success expectations.",
    )
...
def synthesize_product(
    ...
    evidence = {
        ...
        "merger_brief": merger_brief.strip(),
        "mvp_features": [
...
def render_showpiece(evidence: dict[str, Any]) -> str:
    ...
    if evidence.get("merger_brief"):
        lines.extend([
            "## Merger guidance",
            "",
            evidence["merger_brief"],
            "",
        ])
    lines.extend([
        "## Merge ledger",
...
Treat this as the product owner's expectation for what the merged repo should
become. If the source profiles suggest a different direction, document the
tradeoff before changing scope.
"""
...
def main() -> int:
    ...
    if evidence.get("merger_brief"):
        print()
        print("Merger guidance:")
        print(evidence["merger_brief"])
    print()
    print("Source repos:")
    for repo in evidence["source_repos"]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        merger_brief = args.merger_brief
        if args.merger_brief_file is not None:
            merger_brief = args.merger_brief_file.read_text(encoding="utf-8")
        evidence = synthesize_product(
            profile_a,
            profile_b,
            args.product_name,
            args.standalone_repo,
            merger_brief=merger_brief,
        )
